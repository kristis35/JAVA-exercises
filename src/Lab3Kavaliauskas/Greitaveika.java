...
public class Greitaveika {
    ...
    private final SortedSetADTx<Telefonai> aSeries = new BstSetKTUx(new Telefonai(), Telefonai.pagalKaina);
    private final SortedSetADTx<Telefonai> aSeries2 = new BstSetKTUx2(new Telefonai());
    private final SortedSetADTx<Telefonai> aSeries3 = new AvlSetKTUx(new Telefonai());
    ...
    public void SisteminisTyrimas() throws InterruptedException {
        try {
            for (int k : TIRIAMI_KIEKIAI) {
                Telefonai[] telMas = TelGamyba.generuotiIrIsmaisyti(k, 1.0);
                aSeries.clear();
                aSeries2.clear();
                aSeries3.clear();
                tk.startAfterPause();
                tk.start();
                for (Telefonai a : telMas) {
                    aSeries.add(a);
                }
